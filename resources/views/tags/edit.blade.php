...
<head>
    ...
    <title>Edit Tag - Admin Dashboard</title>
    ...
    <div class="max-w-md w-full">
        ...
        <div class="text-center mb-8">
            ...
            <div class="flex items-center justify-center space-x-3 mb-2">
                ...
                <h1 class="text-2xl font-bold text-text">Edit Tag</h1>
            </div>
            <p class="text-text/70">Update tag information</p>
        </div>
        ...
        <div class="bg-secondary p-8 rounded-xl border border-secondary/50 shadow-lg">
            ...
            <form action="{{ route('tags.update', $tag->id) }}" method="POST" class="space-y-6">
                ...
                <div>
                    <label for="name" class="block text-sm font-medium text-text mb-3">Tag Name</label>
                    <div class="relative">
                        ...
                        <div class="absolute inset-y-0 right-0 pr-3 flex items-center">
                            <i class="fas fa-tag text-text/30"></i>
                        </div>
                    </div>
                    <p class="text-text/50 text-sm mt-2">Current tag: <span class="text-accent">{{ $tag->name }}</span></p>
                </div>
        ...
        <!-- Info Card -->
        <div class="mt-6 bg-accent/10 border border-accent/20 rounded-lg p-4">
            <div class="flex items-start">
                <i class="fas fa-info-circle text-accent mt-1 mr-3"></i>
                <div>
                    <h3 class="text-sm font-medium text-text mb-2">Tag Information</h3>
                    <div class="text-sm text-text/70 space-y-1">
                        <p>• Created: {{ $tag->created_at->format('M d, Y') }}</p>
                        <p>• Last updated: {{ $tag->updated_at->format('M d, Y') }}</p>
                    </div>
                </div>
            </div>
        </div>
